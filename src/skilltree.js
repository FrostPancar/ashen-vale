...
function lane(skill, name, icon, kind, key, base, mids) {
  ...
  nodes.push({ id: `${skill}_emp`, tier: 1, slot: 'C', requires: `${skill}_base`, icon, ...emp });
  nodes.push({ id: `${skill}_a`, tier: 2, slot: 'A', requires: `${skill}_emp`, exclusive: `${skill}_b`, icon, ...pA });
  nodes.push({ id: `${skill}_b`, tier: 2, slot: 'B', requires: `${skill}_emp`, exclusive: `${skill}_a`, icon, ...pB });
  nodes.push({ id: `${skill}_syn`, tier: 3, slot: 'C', requires: `${skill}_emp`, icon, ...syn });
...
export const TREES = {
  knight: [
    lane('slash', 'Slash', 'sword', 'Basic', 'LMB',
      ...
      [
        { icon: 'sword+star', name: 'War Tempo', desc: 'Each Slash that lands restores 2 MP and builds 1 Resolve.',
          fx: { resource: { kind: 'resolve', max: 5, perHit: 1, mp: 2 } } },
        { icon: 'sword+burst', name: 'Resolve', desc: 'Taking hits also builds Resolve. At max, your next Slash erupts into a free Cleave.',
          fx: { resolvePayoff: 'cleave', resolveOnHurt: 1 } },
        { icon: 'sword+clock', name: 'Bladedancer', desc: 'Casting a skill spends all Resolve, refunding 0.4s cooldown on your other skills per point.',
          fx: { resolveSpend: 'cdr' } },
        { icon: 'sword+shield', name: 'Vanguard', desc: 'At max Resolve you briefly cannot be staggered, and your next Cleave automatically Sunders.',
    ...
    lane('cleave', 'Cleave', 'cleave', 'Skill 1', 'RMB',
      ...
      [
        { icon: 'cleave+drop', name: 'Rending Edge', desc: 'Cleaved foes Bleed; re-hitting refreshes and stacks it.',
          fx: { apply: [{ status: 'bleed', dur: 3 }] } },
        { icon: 'cleave+clock', name: 'Momentum', desc: 'Each enemy a Cleave touches shaves 0.4s off its cooldown.',
          fx: { cdrPerHit: 0.4 } },
        { icon: 'cleave+crack', name: 'Sunder', desc: 'Cleaved foes lose armor — Sundered enemies take 30% more damage.',
          fx: { apply: [{ status: 'sunder', dur: 5 }] } },
        { icon: 'cleave+skull', name: "Executioner's Arc", desc: 'Cleave deals double damage to Bleeding or Sundered foes under 35% HP, and refunds its MP on a kill.',
    ...
    lane('whirl', 'Whirlwind', 'whirl', 'Skill 2', 'SPACE',
      ...
      [
        { icon: 'whirl+spiral', name: 'Cyclone', desc: 'The spin vacuums nearby enemies inward before it hits.',
          fx: { buff: { type: 'vacuum', power: 2.6 } } },
        { icon: 'whirl+ring', name: 'Bladestorm', desc: 'Leaves a lingering ring of cutting wind that keeps shredding foes.',
          fx: { hazard: { kind: 'wind', dur: 3, radius: 1.9, dmgMult: 0.4, period: 0.5 } } },
        { icon: 'whirl+shield', name: 'Iron Tempest', desc: 'For each foe the spin hits, gain a stack of damage reduction for 4s.',
          fx: { guardPerHit: { dur: 4, perHit: 0.08, max: 0.4 } } },
        { icon: 'whirl+heart', name: 'Bloodspin', desc: 'Whirlwind consumes Bleed on every foe it hits, healing you 6 per stack.',
    ...
    lane('rush', 'Shield Rush', 'bash', 'Mobility', 'SHIFT',
      ...
      [
        { icon: 'bash+swap', name: 'Phalanx', desc: 'After the charge, raise a shield that reflects projectiles for 1.5s.',
          fx: { buff: { type: 'reflect', dur: 1.5 } } },
        { icon: 'bash+burst', name: 'Concussive', desc: 'Stunned foes are Dazed — rooted and taking 40% more damage.',
          fx: { apply: [{ status: 'daze', dur: 2 }] } },
        { icon: 'bash+sparkle', name: 'Unstoppable', desc: 'Rush cleanses crowd control and grants immunity through the charge.',
          fx: { buff: { type: 'unstoppable', dur: 0.6 }, cleanse: true } },
        { icon: 'bash+wave', name: 'Aftershock', desc: 'Rush ends in a shockwave; crash into a Sundered foe and it knocks down the whole pack.',
  ...
  ranger: [
    lane('arrow', 'Quick Shot', 'bow', 'Basic', 'LMB',
      ...
      [
        { icon: 'bow+star', name: 'Rhythm', desc: 'Keep firing — every 3rd Quick Shot deals double damage.',
          fx: { resource: { kind: 'cadence', max: 3, perHit: 1, payoffMult: 2 } } },
        { icon: 'bow+link', name: 'Predator', desc: 'Quick Shots into a Marked foe splash to a nearby enemy and refund 3 MP.',
          fx: { chain: { vsStatus: 'mark', mult: 0.5, radius: 2.2, mp: 3 } } },
        { icon: 'bow+drop', name: 'Bodkin', desc: 'Critical Quick Shots open a short Bleed.',
          fx: { applyOnCrit: [{ status: 'bleed', dur: 2.5 }] } },
        { icon: 'bow+crosshair', name: 'Open Season', desc: 'Killing a Marked foe spreads the Mark to nearby enemies.',
          fx: { markSpread: 2.6 } },
      ]),
    lane('pshot', 'Power Shot', 'pshot', 'Skill 1', 'RMB',
      { name: 'Power Shot', desc: 'Heavy bolt that pierces a line of enemies.' },
      [
        { icon: 'pshot+crosshair', name: "Hunter's Mark", desc: 'The first foe pierced is Marked — takes 25% more damage from everything.',
          fx: { apply: [{ status: 'mark', dur: 6 }], firstOnly: true } },
        { icon: 'pshot+snow', name: 'Pinning Shot', desc: 'Pierced foes are Chilled and pinned in place.',
          fx: { apply: [{ status: 'chill', dur: 2.5 }] } },
        { icon: 'pshot+burst', name: 'Overdraw', desc: 'Power Shot detonates on impact in a small blast.',
          fx: { hazard: { kind: 'blast', dur: 0.1, radius: 1.4, dmgMult: 0.8, period: 0.1, atHit: true } } },
        { icon: 'pshot+clock', name: 'Skewer', desc: 'A Power Shot through a Marked foe instantly resets Multishot.',
    ...
    lane('mshot', 'Multishot', 'mshot', 'Skill 2', 'SPACE',
      ...
      [
        { icon: 'mshot+plus', name: 'Splinter', desc: 'Fires 2 extra arrows in the fan.',
          fx: { proj: 2 } },
        { icon: 'mshot+rain', name: 'Suppressing Volley', desc: 'Leaves an arrow-pelted field that slows enemies who cross it.',
          fx: { hazard: { kind: 'field', dur: 4, radius: 2.0, dmgMult: 0.2, period: 0.6, slow: 0.5 } } },
        { icon: 'mshot+focus', name: 'Focused Fan', desc: 'Tightens the spread so the whole fan can land on one target.',
          fx: { proj: 2, narrow: 0.4 } },
        { icon: 'mshot+trap', name: 'Volley Trap', desc: 'Multishot also drops a trap that snares the first foe to cross it.',
    ...
    lane('tumble', 'Tumble', 'tumble', 'Mobility', 'SHIFT',
      ...
      [
        { icon: 'tumble+cloud', name: 'Smoke Roll', desc: 'Leave a smoke cloud and become Unseen — your next attack is empowered.',
          fx: { buff: { type: 'unseen', dur: 4 }, hazard: { kind: 'smoke', dur: 3, radius: 1.6, dmgMult: 0, period: 1, atSelf: true } } },
        { icon: 'tumble+trap', name: 'Trapper', desc: 'End the roll by dropping a snare that roots the first enemy in.',
          fx: { hazard: { kind: 'trap', dur: 6, radius: 1.1, dmgMult: 0.3, period: 0.2, root: true, atSelf: true } } },
        { icon: 'tumble+clock', name: 'Acrobat', desc: 'A kill during the invuln window instantly refreshes Tumble.',
          fx: { refreshOnKillDuringIframes: true } },
        { icon: 'tumble+eye', name: 'Ambush', desc: 'Your first hit from Unseen is a guaranteed crit and applies Mark.',
  ...
  mage: [
    lane('spark', 'Spark', 'staff', 'Basic', 'LMB',
      ...
      [
        { icon: 'staff+orb', name: 'Arcane Charges', desc: 'Each Spark banks a charge; at 3, your next skill is free and empowered.',
          fx: { resource: { kind: 'charge', max: 3, perHit: 1, payoff: 'empower' } } },
        { icon: 'staff+swap', name: 'Resonance', desc: 'Spark applies Charged to foes already Chilled, and Chill to those already Charged.',
          fx: { resonance: true } },
        { icon: 'staff+flame', name: 'Kindle', desc: 'Spark leaves a light Burn.',
          fx: { apply: [{ status: 'burn', dur: 3 }] } },
        { icon: 'staff+burst', name: 'Elemental Confluence', desc: 'Any foe carrying 2+ elemental statuses detonates for an arcane blast.',
          fx: { detonate: true } },
      ]),
    lane('bolt', 'Spark Bolt', 'bolt', 'Skill 1', 'RMB',
      { name: 'Spark Bolt', desc: 'Heavy single-target bolt.' },
      [
        { icon: 'bolt+zap', name: 'Overcharge', desc: 'Hit foes become Charged; lightning arcs from them to neighbors.',
          fx: { apply: [{ status: 'charged', dur: 4 }], chain: { vsStatus: 'charged', mult: 0.5, radius: 2.4 } } },
        { icon: 'bolt+orb', name: 'Conduit', desc: 'Leaves a hovering orb that zaps nearby enemies for a few seconds.',
          fx: { hazard: { kind: 'orb', dur: 4, radius: 2.2, dmgMult: 0.35, period: 0.5, atHit: true } } },
        { icon: 'bolt+snow', name: 'Static Field', desc: 'Bolt Chills and applies Shock — the next hit on a Shocked foe stuns.',
          fx: { apply: [{ status: 'chill', dur: 3 }, { status: 'shock', dur: 4 }] } },
        { icon: 'bolt+cloud', name: 'Thunderhead', desc: 'Charged foes hit by Bolt erupt into a roaming lightning storm.',
    ...
    lane('nova', 'Nova', 'nova', 'Skill 2', 'SPACE',
      ...
      [
        { icon: 'nova+snow', name: 'Frost Nova', desc: 'Nova Chills and can Freeze; frozen foes take 50% more damage and shatter.',
          fx: { apply: [{ status: 'chill', dur: 3 }, { status: 'frozen', dur: 1.5 }] } },
        { icon: 'nova+ring', name: 'Echo Nova', desc: 'A second, smaller ring pulses 0.7s later.',
          fx: { pulse: { delay: 0.7, frac: 0.6 } } },
        { icon: 'nova+spiral', name: 'Singularity', desc: 'Drag enemies inward into a gravity well before the ring bursts.',
          fx: { buff: { type: 'vacuum', power: 3 } } },
        { icon: 'nova+zap', name: 'Flash Freeze', desc: 'Charged foes caught by Nova freeze solid and stay Shocked.',
          fx: { apply: [{ status: 'frozen', dur: 2 }], vsStatus: 'charged' } },
      ]),
    lane('blink', 'Blink', 'blink', 'Mobility', 'SHIFT',
      { name: 'Blink', desc: 'Short instant teleport.' },
      [
        { icon: 'blink+burst', name: 'Arcane Wake', desc: 'The point you leave from collapses into a small implosion.',
          fx: { hazard: { kind: 'implode', dur: 0.3, radius: 1.8, dmgMult: 0.7, period: 0.3, atOrigin: true } } },
        { icon: 'blink+sparkle', name: 'Phase', desc: 'Brief invulnerability through the blink; your next spell costs no mana.',
          fx: { buff: { type: 'freecast', dur: 4 }, iframes: 0.6 } },
        { icon: 'blink+plus', name: 'Doublecast', desc: 'Blink stores up to 2 charges for chained teleports.',
          fx: { charges: 2 } },
        { icon: 'blink+rune', name: 'Displacement', desc: 'Blinking through foes Runes them; your next Nova detonates every Rune.',
          fx: { runePass: true } },
      ]),
  ],
};
